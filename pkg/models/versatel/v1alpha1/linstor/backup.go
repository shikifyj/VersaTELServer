...
func GetBackupData() []map[string]interface{} {
    sc, _ := GetIPAndConnect(22)
    cmd := "linstor schedule list-by-resource --active-only | awk 'BEGIN{FS=\"|\"; OFS=\";\"} NR>2 {print $2, $3, $4, $5, $6}'"
    out, err := SshCmd(sc, cmd)
    if err != nil {
        errInfo := fmt.Sprintf(strings.Replace(strings.TrimSpace(out), "\n", "", -1))
        Message := client.ApiCallError{client.ApiCallRc{Message: errInfo}}
    ...
    for _, line := range lines {
        ...
        backupInfo := map[string]interface{}{
            "resName":      strings.TrimSpace(fields[0]),
            "remoteName":   strings.TrimSpace(fields[1]),
            "scheduleName": strings.TrimSpace(fields[2]),
            "lastPlan":     strings.TrimSpace(fields[3]),
            "nextPlan":     strings.TrimSpace(fields[4]),
        }
